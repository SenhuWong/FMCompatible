...
void Brick4Cobalt3D::reOrdering(double* xyz)
{
	int nTri = 0;
	int nQuadri = 0;
	std::set<int,std::less<int>> allNodes;
	for (int i = 0; i < nface; i++)
	{
		int nnode = nnodes[i];
		for (int j = 0; j < nnode; j++)
		{
			allNodes.insert(eachNodes[i][j]);
		}
	}
	double xc[3] = {0,0,0};
	...
	xc[0] = xc[0] / allNodes.size();
	xc[1] = xc[1] / allNodes.size();
	xc[2] = xc[2] / allNodes.size();
...
//(0,6) brick
	if (nTri == 4 and nQuadri == 0)
	{
		...
		int bottom = 0;
		GeomElements::vector3d<3,double> cell_center(xc);
		GeomElements::vector3d<3,double> A(&xyz[3*(eachNodes[bottom][0]-1)]);
		GeomElements::vector3d<3,double> B(&xyz[3*(eachNodes[bottom][1]-1)]);
		GeomElements::vector3d<3,double> C(&xyz[3*(eachNodes[bottom][2]-1)]);
		GeomElements::vector3d<3,double> AB = B-A;
		GeomElements::vector3d<3,double> AC = C-A;
		GeomElements::vector3d<3,double> edge_center = (A + B + C) / 3;
		GeomElements::vector3d<3,double> edge_normal = AB.cross_product(AC);
		if((cell_center-edge_center).dot_product(edge_normal) > 0)
		{
			eachNodesDir[bottom] = 1;
		}
		else
		{
			eachNodesDir[bottom] = -1;
	...
	else if (nTri == 4 and nQuadri == 1)//Pyramid
	{
		...
		GeomElements::vector3d<3,double> cell_center(xc);
		GeomElements::vector3d<3,double> A(&xyz[3*eachNodes[bottom][0]-3]);
		GeomElements::vector3d<3,double> B(&xyz[3*eachNodes[bottom][1]-3]);
		GeomElements::vector3d<3,double> C(&xyz[3*eachNodes[bottom][2]-3]);
		GeomElements::vector3d<3,double> AB = B-A;
		GeomElements::vector3d<3,double> AC = C-A;
		GeomElements::vector3d<3,double> edge_center = (A + B + C) / 3;
		GeomElements::vector3d<3,double> edge_normal = AB.cross_product(AC);
