...
void ADT::buildADT(int d, int nelements, double* elementBbox)
{
    ...
    for (int j = 0; j < nelem; j++)
    {
        ...
        {
            int i2 = 2 * i;
            adtExtents[i2] = std::min(adtExtents[i2], coord[jd + i]);            
        }
        for (int i = 0; i < ndim / 2; i++)
        {
            int i2 = 2 * i + 1;
            adtExtents[i2] = std::max(adtExtents[i2], coord[jd + i + ndim / 2]);
        }
    }
    tolerance = 0.01;
    for (int i = 0; i < ndim / 2; i++)
    ...
    delete[] elementsAvailable;
    elementsAvailable = nullptr;
    delete[] adtWork;
    adtWork = nullptr;
...
// void ADT::dummySearch(MeshBlock* mb,int* cellIndex, double*xsearch)
// {
//     for(int i = 0;i<nelem;i++)
//     {
//         int i4 = adtIntegers[4*i];
//         mb->checkContainment(cellIndex,i4,xsearch);
//         if(cellIndex[0] >= 0)
//         {
//             break;
//         }
